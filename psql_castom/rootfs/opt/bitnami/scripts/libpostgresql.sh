...
postgresql_initialize() {
    ...
    if ! is_dir_empty "$POSTGRESQL_DATA_DIR"; then
        ...
        [[ "$HOSTNAME" = "postgres-0" ]] && [[ -n "$POSTGRESQL_REPLICATION_USER" ]] && is_boolean_yes "$create_pghba_file" && postgresql_add_replication_to_pghba
        [[ "$HOSTNAME" = "postgres-0" ]] && is_boolean_yes "$create_pghba_file" && postgresql_configure_synchronous_replication
        [[ "$HOSTNAME" = "postgres-1" ]] && postgresql_configure_recovery
    else
        if [[ "$HOSTNAME" = "postgres-0" ]]; then
